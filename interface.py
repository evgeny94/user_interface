...
#
# geometry = str(screen_width1) + "x" + str(screen_height1)
#
...
screen_width = int(int(frame_left.winfo_screenwidth())*0.85)
screen_height = int(int(frame_left.winfo_screenheight())) #*1 I have played with this there is no change so we keep it on normal
...
def clicked(value):
    ...
    if value == "Input of amount":
        ...
        #we don't need to put need button all the time we can change one that alreday made
...
def submit_entry():
    ...
    print(ge)
    print(before_cutting_num)
    final_confirmation['state'] = NORMAL
    if num_clicked == 1: #Why we needed the num check?
        ...
        # Leading Sansan not found
            ...
            ##-------------We need to think what is the right timming to enable cut button---------------------------
    # Already clicked
    else:
        submit_restart()
        found_respond = messagebox.askyesno("Manual decision about the <Leading Sansan>",
        ...
        # Leading Sansan not found
            ...
            ##-------------We need to think what is the right timming to enable cut button---------------------------
    #else:
    #    bigger_then_sonar = messagebox.showerror("Number Error","You have writh a bigger number then the soner gave\n this is not possibale")
    #    num_clicked = 0
...
##--------------------------Right Top 1 Frame-------------------------##
before_cutting_num =random.randint(3000, 5000)

top1_right_frame = LabelFrame(frame_right, text="Assessmention Section")
top1_right_frame.grid(row=0, column=0, sticky="nsew")

assessment_label = Label(top1_right_frame, text="Dates Assessment Before Cutting: " + str(before_cutting_num))
...
Radiobutton(top2_right_frame, text="Manual marking on the image", variable=r, value="Manual marking",
            command=lambda : clicked(r.get())).grid(row=2, column=0, sticky="w")
Radiobutton(top2_right_frame, text="Enter input for the amount that will remain", variable=r, value="Input of amount",
            command=lambda : clicked(r.get())).grid(row=3, column=0, sticky="w")
